...
// 🎀 Shërbe skedarët statikë të frontend-it - KJO HIQET, NGINX DO TA BËJË
...
io.on("connection", (socket) => {
  ...
  socket.on("supportRequest", (data) => {
    console.log(
      "🆘 Kërkesa për ndihmë u mor (server side - kjo socket emit mund të mos jetë e nevojshme):",
      data
    );
    ...
    supportLogsRef
      ...
      .then(() => {
        console.log(
          "✅ Kërkesa për ndihmë u ruajt me sukses në Firebase (server side)!"
        );
      })
      .catch((error) => {
        console.error(
          "❌ Gabim gjatë ruajtjes së kërkesës në Firebase (server side):",
          error
        );
      });
  });
